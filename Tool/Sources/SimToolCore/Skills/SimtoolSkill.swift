    ...
    static let simtoolCartographMarkdown = #"""
        ...
        **8. Tap and descend.** Pick an untried tappable element (Buttons, Cells,
        ...
        - Node — required: `id`, `title`, `fingerprint`, `screenshot`, `depth`,
          ...
          `groups`, `entryPoint`, `longShot`. `fingerprint` is the robot's structural
          hash and `key` its screen-identity hash — the robot resumes by them; in an
          agent pass any stable unique string works for both — reuse the node id.
