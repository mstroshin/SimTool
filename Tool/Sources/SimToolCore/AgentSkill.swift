...
/// The bundled `simtool` agent skill: a Markdown brief that teaches a coding
/// agent how to drive this CLI. Embedded as a string rather than a SwiftPM
/// resource so the Homebrew-installed binary stays a single self-contained
/// file with no bundle to locate at runtime.
public enum AgentSkill {
    /// Where an installed skill lives, in Claude Code's conventional layout.
    public enum Scope: String, CaseIterable, Sendable {
        /// `<project>/.claude/skills/simtool/SKILL.md` — this checkout only.
        case local
        /// `~/.claude/skills/simtool/SKILL.md` — every project on this machine.
        case global
        /// Install nothing.
        case none
    }
    ...
    public static let skillName = "simtool"
    public static let fileName = "SKILL.md"

    /// Directory the skill is written into for `scope`, or `nil` for `.none`.
    /// `home` is injectable so tests never touch the real `$HOME`.
    ...
    /// Writes `SKILL.md` for `scope`. Never clobbers a file whose contents
    /// differ from the bundled skill unless `force` is set — an edited skill is
    /// the user\'s, and `simtool init` is re-run often enough that silently
    ...
    public static func install(
        ...
        guard let directory = directory(for: scope, projectDirectory: projectDirectory, home: home) else {
            return Installation(scope: scope.rawValue, outcome: .skipped, path: nil)
        }
        ...
        if let existing {
            if existing == markdown {
                return Installation(scope: scope.rawValue, outcome: .upToDate, path: path)
            }
            guard force else {
                return Installation(scope: scope.rawValue, outcome: .conflict, path: path)
            }
        }
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        try Data(markdown.utf8).write(to: destination, options: [.atomic])
        return Installation(scope: scope.rawValue, outcome: existing == nil ? .created : .updated, path: path)
    }

    /// The skill document itself. Mirrored by `skills/simtool/SKILL.md` in the
    /// repository (kept in sync by `AgentSkillTests`), which is the copy to edit
    /// when authoring — regenerate this literal from it.
